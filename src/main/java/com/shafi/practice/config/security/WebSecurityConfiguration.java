...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http
			.authorizeRequests()
			.antMatchers("/images/**","/css/**", "/js/**","/font/**","/register","/profile").permitAll()
				 ...
				 * .antMatchers("/country/inactive").hasRole("ADMIN")
				 * .antMatchers("/manager/add").hasRole("ADMIN")
				 * .antMatchers("/team/add").hasRole("ADMIN")
				 */
//			.antMatchers("/country/edit").hasRole("ADMIN")
				/*
				 * .antMatchers("/manager/details").hasRole("ADMIN")
				 * .antMatchers("/manager/show-all").hasRole("ADMIN")
				 * .antMatchers("/player/add").hasRole("MANAGER")
				 * .antMatchers("/player/**").hasRole("MANAGER")
				 * .antMatchers("/api/v1/**").hasRole("MANAGER")
				 * .antMatchers("/team/show").hasAnyRole("ADMIN","MANAGER")
				 */
//			.antMatchers("/country/show-all").hasAnyRole("ADMIN", "MANAGER")
